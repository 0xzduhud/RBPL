...
$totRev = $pdo->query("SELECT COALESCE(SUM(total),0) FROM orders WHERE status='delivered'")->fetchColumn();
$totOrd = $pdo->query("SELECT COUNT(*) FROM orders")->fetchColumn();
$totEmp = $pdo->query("SELECT COUNT(*) FROM users WHERE role='employee' AND status='active'")->fetchColumn();
?>
  ...
  <div class="bg-gradient-to-r from-orange-500 to-red-500 text-white p-6 rounded-b-3xl">
    <div class="flex items-center gap-4 mb-4">
      ...
      <div>
        ...
        <p class="text-orange-100 text-sm">Pemilik · @<?= htmlspecialchars($user['username']) ?></p>
      </div>
    </div>
    <div class="grid grid-cols-3 gap-3">
      <div class="bg-white/20 rounded-xl p-3 text-center"><p class="text-lg font-bold"><?= number_format($totRev/1000000,1) ?>jt</p><p class="text-xs opacity-80">Revenue</p></div>
      <div class="bg-white/20 rounded-xl p-3 text-center"><p class="text-lg font-bold"><?= $totOrd ?></p><p class="text-xs opacity-80">Pesanan</p></div>
      <div class="bg-white/20 rounded-xl p-3 text-center"><p class="text-lg font-bold"><?= $totEmp ?></p><p class="text-xs opacity-80">Karyawan</p></div>
    </div>
  </div>

  <?php if ($msg): ?>
  <div class="mx-4 mt-4 p-3 bg-green-100 border border-green-300 rounded-xl text-sm text-green-700 flex gap-2"><i data-lucide="check-circle" class="w-4 h-4"></i><?= htmlspecialchars($msg) ?></div>
  <?php endif; ?>
